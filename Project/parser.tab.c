...
{
  "\"end of file\"", "error", "\"invalid token\"", "ASSIGN", "PRINT",
...
#line 32 "parser.y"
                                       { 
        insertSymbol((yyvsp[-3].var), (yyvsp[-1].num));
        printf("Assigned: %s = %d\n", (yyvsp[-3].var), (yyvsp[-1].num));
    }
...
#line 36 "parser.y"
                                 {
        int value = getSymbolValue((yyvsp[-1].var));
        if (value != -1)
            printf("Variable %s = %d\n", (yyvsp[-1].var), value);
        else
